...
function AdminLoginData() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm({ mode: 'onChange' }); // Initialize useForm for form validation
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const response = await apiRequest({
        url: '/auth/adminlogin', // Replace with your admin login API endpoint
        method: 'POST',
        data,
      });
    ...
    <div className="min-h-screen   flex items-center justify-center border-b bg-[#287ec445]">
      <div className="bg-blue p-8 rounded shadow-md w-full max-w-md ">
        ...
        <form onSubmit={onSubmit}>
          <div className="mb-4">
            <label className="block text-gray-700 font-bold mb-2 text-white" htmlFor="username">
              Username
            </label>
            <input
              type="text"
